# Extract timings from runlog files.
#
# Hongyang Zhou, hyzhou@umich.edu

using DataFrames, Printf, Query

function extractTestInfo(File::AbstractString, compiler::SubString="unknown")
...
for i in reverse(eachindex(lines))
   iStart = 0
   if !IsComplete && (startswith(lines[i]," error report:") ||
      startswith(lines[i],"Error_report"))
