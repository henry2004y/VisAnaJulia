pro extract_galileo_times, $
  ...
  if n_elements(freqrange) eq 0 and (n_elements(xrange) ne 2 or n_elements(yranges) ne 8) then begin
     case flyby of
        ...
        8: begin
           xrange  = [945, 967]
           ;yranges = [[-100,200],[-150,150],[-150,150],[0,200]]
  ...
  for ifile = 0, nfile - 1 do begin
     ...
     for ipict = 0, maxpict-1 do begin
        ...
        case ifile of
           0: begin
              time0(ipict) = time
              b0(*,0,ipict) = interpolate(w(*,*,*,4), isat, jsat, ksat)
              b0(*,1,ipict) = interpolate(w(*,*,*,5), isat, jsat, ksat)
              b0(*,2,ipict) = interpolate(w(*,*,*,6), isat, jsat, ksat)
              b0(*,3,ipict) = sqrt(b0(*,0,ipict)^2 + b0(*,1,ipict)^2 + b0(*,2,ipict)^2)
           end
           1: begin
              time1(ipict) = time
              b1(*,0,ipict) = interpolate(w(*,*,*,4), isat, jsat, ksat)
              b1(*,1,ipict) = interpolate(w(*,*,*,5), isat, jsat, ksat)
              b1(*,2,ipict) = interpolate(w(*,*,*,6), isat, jsat, ksat)
              b1(*,3,ipict) = sqrt(b1(*,0,ipict)^2 + b1(*,1,ipict)^2 + b1(*,2,ipict)^2)
           end
           2: begin
              time2(ipict) = time
              b2(*,0,ipict) = interpolate(w(*,*,*,4), isat, jsat, ksat)
              b2(*,1,ipict) = interpolate(w(*,*,*,5), isat, jsat, ksat)
              b2(*,2,ipict) = interpolate(w(*,*,*,6), isat, jsat, ksat)
  ...
  for timeshift = timeshift0, timeshift1 do begin
     ...
     wlog =datasat      
     ;wlog(*,0) = wlog(*,0) +80; hyzhou time shift for satellite, G29
     wlog1=datamodel0
     wlog2=datamodel1
     wlog3=datamodel2
     if n_elements(freqrange) gt 0 then begin
